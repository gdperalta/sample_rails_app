...
======*/
resource "aws_cloudwatch_log_group" "sample_rails_app" {
  name = "sample_rails_app"

  tags = {
    Environment = "${var.environment}"
    Application = "sample_rails_app"
  }
}
...
======*/
resource "aws_ecr_repository" "sample_rails_app" {
...
data "template_file" "web_task" {
  ...
  vars = {
    image           = "${aws_ecr_repository.sample_rails_app.repository_url}"
    ...
    log_group       = "${aws_cloudwatch_log_group.sample_rails_app.name}"
    region          = "${var.region}"
  }
}
...
resource "aws_alb" "alb_sample_rails_app" {
  name            = "${var.environment}-alb-sample-rails-app"
  subnets         = "${var.public_subnet_ids}"
  security_groups = "${concat(var.security_groups_ids, [aws_security_group.web_inbound_sg.id])}"

  tags = {
    Name        = "${var.environment}-alb-sample_rails_app"
    Environment = "${var.environment}"
  }
}

resource "aws_alb_listener" "sample_rails_app" {
  load_balancer_arn = "${aws_alb.alb_sample_rails_app.arn}"
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  alarm_name          = "${var.environment}_sample_rails_app_web_cpu_utilization_high"
