resource "aws_s3_bucket" "source" {
  bucket        = "sample-rails-app-experiment-source"
  acl           = "private"
  force_destroy = true
}
...
resource "aws_codebuild_project" "sample_rails_app_build" {
  name          = "sample_rails_app-codebuild"
  build_timeout = "10"
...
resource "aws_codepipeline" "pipeline" {
  name     = "sample_rails_app-pipeline"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        FullRepositoryId = "gdperalta/sample_rails_app"
        BranchName     = "main"
        ConnectionArn    = "${aws_codestarconnections_connection.example.arn}"
        # Owner      = "gdperalta"
        # Repo       = "sample_rails_app"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ProjectName = "sample-rails-app-codebuild"
      }
    }
  }
